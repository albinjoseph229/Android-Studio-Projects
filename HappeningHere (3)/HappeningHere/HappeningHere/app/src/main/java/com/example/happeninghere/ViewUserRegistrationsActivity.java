...
public class ViewUserRegistrationsActivity extends AppCompatActivity {
    ...
    private void loadRegistrations() {
        ...
        // Updated SQL query to also fetch the 'place' column
        Cursor cursor = db.rawQuery("SELECT events.id AS _id, events.name, events.description, events.date, events.place " +
                "FROM events " +
                "INNER JOIN registrations ON events.id = registrations.event_id " +
                "WHERE registrations.user_id = ?", new String[]{String.valueOf(userId)});

        if (cursor != null) {
            Log.d(TAG, "Number of registered events: " + cursor.getCount());
            // Added "place" to the 'from' array
            String[] from = {"_id", "name", "description", "date", "place"};
            // Added R.id.eventPlaceTextView to the 'to' array
            int[] to = {R.id.eventIdTextView, R.id.eventNameTextView, R.id.eventDescriptionTextView, R.id.eventDateTextView, R.id.eventPlaceTextView};
